...
import { Target, Shield, Zap, Save, Trophy, User, Check, Minus, Plus, Map, Goal, X as CloseIcon, FileText, Download, History, ChevronDown, ChevronUp, Trash2, ShieldX, ShieldAlert, Crosshair, Clock, UserPlus, AlertCircle, Star, Gamepad2 } from 'lucide-react';
import { useGoalkeepers } from '../context/GoalkeeperContext';
import { MatchScout } from '../types';
import GameScout from '../src/components/GameScout';
...
const ScoutPage: React.FC = () => {
  ...
  const [scoutType, setScoutType] = useState<'technical' | 'game'>('game');
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 max-w-[1400px] mx-auto pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-4 bg-card border border-gray-800 rounded-full p-1 w-min">
            <button 
              onClick={() => setScoutType('game')}
              className={`px-4 py-1.5 text-[10px] font-black uppercase rounded-full flex items-center gap-2 transition-all ${scoutType === 'game' ? 'bg-gold text-black shadow-md' : 'text-gray-500 hover:text-white'}`}>
              <Gamepad2 size={14}/>
              Scout de Jogo
            </button>
            <button 
              onClick={() => setScoutType('technical')}
              className={`px-4 py-1.5 text-[10px] font-black uppercase rounded-full flex items-center gap-2 transition-all ${scoutType === 'technical' ? 'bg-gold text-black shadow-md' : 'text-gray-500 hover:text-white'}`}>
              <Target size={14}/>
              Scout Técnico
            </button>
          </div>
